    ...
    try:
        ...
        while len(bytes(data, 'utf-8')) != DATA_LENGTH:
            ...
            if cmd[0] == 'ls':
                ...
                bytesSent = 0
                dirSize = convert_data_str(len(response),data_size)
                ...
                if not os.path.isfile(cmd[1]):
                    ...
                    plsSend = convert_data_str("YES",data_size)
                ...
                if os.path.isfile(cmd[1]):
                    ...
                    sizeSend = convert_data_str(sfileSize,data_size)
                    connectionSocket.send(bytes(sizeSend,'utf-8'))
                    while bytesSent < fileSize:
                        buff = get_file.read(data_size)
                        connectionSocket.send(buff)
                        bytesSent += data_size
                    get_file.close()
                    print("file closed, waiting for receipt")
                    receipt = connectionSocket.recv(data_size).decode('utf-8')
                    print ("File" + receipt)
                else:
                    skipSend = convert_data_str("NO",data_size)
                    connectionSocket.send(bytes(skipSend,'utf-8'))
