...
while user_cmd[0] != "exit":
    if user_cmd[0] == "get":
        ...
        if not os.path.isfile(user_cmd[1]):
            ...
            receipt = dataSocket.recv(data_size)
            ...
            if proceed != "NO":
                ...
                #receive file
                fileBytesSent = 0
                fileSize = dataSocket.recv(data_size)
                fSize = int(fileSize.decode('utf-8').strip('\x00'))
                while fileBytesSent < fSize:
                        tmp2Buff = dataSocket.recv(data_size)
                        if not tmp2Buff:
        ...
        if proceed == "NO":
            ...
            dataSocket.send(bytes(sizeSend,'utf-8'))
            while bytesSent < fileSize:
                buff = put_file.read(data_size)
                sendData = convert_data_str(buff,data_size)
                dataSocket.send(bytes(sendData,'utf-8'))
                bytesSent += data_size
            put_file.close()
            print("file closed, waiting for receipt")
            receipt = dataSocket.recv(data_size).decode('utf-8')
            print ("File" + receipt)
        dataSocket.close()
    elif user_cmd[0] == "ls":
        ...
        #receive size
        dirSize = dataSocket.recv(data_size)
